...
export type TypePredExp = {tag: "TypePredExp"; param: VarDecl[]; type: CExp[]; }
export const makeTypePredExp = (param: VarDecl[], type: CExp[]): TypePredExp => ({tag: "TypePredExp", param: param, type: type});
export const isTypePredExp = (x: any): x is TypePredExp => x.tag === "TypePredExp";
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        const body = mapResult(parseL5CExp, rest[0] === ":" ? rest[0] === "is?" ? rest.slice(3) : rest.slice(2) : rest);
        const returnTE = rest[0] === ":"?rest[1] === "is?"?bind(parseTExp(['is?', rest[2]]),(x:TExp)=>makeOk(makeTypePredTExp(x)))://added for typepred
