...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         ...
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isSubType,TypePredTExp,isTypePredTExp } from "./TExp";
...
export const typeofPrim = (p: PrimOp): Result<TExp> =>
    ...
    (p.op === 'eq?') ? parseTE('(any * any -> boolean)') :
    (p.op === 'string=?') ? parseTE('(any * any -> boolean)') :
    (p.op === 'display') ? parseTE('(any -> void)') :
    (p.op === 'newline') ? parseTE('(never -> void)') :
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<TypePredTExp> => {
    if(!isAppExp(e)){
        return makeFailure("Fail: not AppExp in isTypePredApp");
    }
    const typeO = typeofExp(e.rator,tenv);
    if(isOk(typeO)&&isProcTExp(typeO.value)&&isTypePredTExp(typeO.value.returnTE)){
        return makeOk(typeO.value.returnTE);
    }
    return makeFailure("Fail: not ok or procExp or TypePredTExp in isTypePredApp");
}

export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>{
    return either(
            bind (isTypePredApp(ifExp.test, tenv), (type: TypePredTExp) => typeOfIfPred(ifExp,tenv,type)),
            makeOk,
            () => typeofIfNormal(ifExp, tenv));
}


export const typeOfIfPred=(ifExp: IfExp, tEnv: TEnv, typePred:TypePredTExp): Result<TExp> => {
        const then = typeofExp(ifExp.then, tEnv);
        const alt = typeofExp(ifExp.alt, tEnv);
        return bind(then, (then: TExp) =>
                    bind(checkCompatibleType(typePred.type,then,ifExp),(r:true)=>
                        bind(alt, (alt: TExp) =>
                            makeOk(makeUnion(typePred.type, alt)))));
};
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    bind(typeofExp(app.rator, tenv), (ratorTE: TExp) => {
        ...
        return makeFailure("Fail: typeofApp, rator not procTExp");
    });
