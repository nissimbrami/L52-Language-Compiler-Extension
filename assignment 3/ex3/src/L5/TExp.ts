...
export type CompoundTExp = ProcTExp | TupleTExp | UnionTExp | InterTExp | DiffTExp | TypePredTExp;
export const isCompoundTExp = (x: any):   x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isUnionTExp(x) || isInterTExp(x) ||isDiffTExp(x) ||isTypePredTExp(x);
...
export const makeInterTExp = (tes: TExp[]) :TExp =>{
    return dnf(normalizeInter(({tag: "InterTExp"  , components: flattenSortInter(tes)})));
}
export const isInterTExp = (x:any): x is InterTExp  => x.tag === "InterTExp";


const normalizeInter = (inter:InterTExp):TExp =>
    isEmpty(inter.components) ? makeNeverTExp():
    includes(makeNeverTExp(),inter.components) ? makeNeverTExp():
    inter.components.length ==1 ?inter.components[0]:
    filter(isUnionTExp,inter.components)?  dnf(inter):
    filter(isInterTExp,inter.components) ? dnf(inter) :
    makeNeverTExp();
...
const removeDuplicatesAndAny = (s: TExp[]): TExp[] =>{
    return isEmpty(s) ? s :
    isAnyTExp(s[s.length-1]) ? removeDuplicatesAndAny(s.slice(0,s.length-1)) :
    containsType(s.slice(0,s.length-1), s[s.length-1]) ? removeDuplicatesAndAny(s.slice(0,s.length-1)) :
    [ ...removeDuplicatesAndAny(s.slice(0,s.length-1)),s[s.length-1]];
}


export type DiffTExp = {tag:"diffTExp";te1:TExp;te2:TExp}
export const makeDiffTExp = (te1:TExp,te2:TExp):TExp =>
normalizeDiff(te1,te2);
export const isDiffTExp = (x:any):x is DiffTExp => x.tag==="diffTExp";



export const normalizeDiff = (te1:TExp,te2:TExp):TExp=>
isAtomicTExp(te1)&& !isSubType(te1,te2)?te1:
isUnionTExp(te1) && isUnionTExp(te2)? makeUnionTExp(differenceWith((t1:TExp,t2:TExp)=>
    superTypeComparator(t1,t2)===0,te1.components,te2.components)):
isUnionTExp(te1) ? makeUnionTExp(differenceWith((t1:TExp,t2:TExp)=>superTypeComparator(t1,t2)===0,te1.components,[te2])):
isAnyTExp(te2) ? makeNeverTExp():
isAnyTExp(te1) ? makeAnyTExp():
isSubType(te1,te2) ? makeNeverTExp():
{tag:"diffTExp",te1:te1,te2:te2}
...
//if i have inter(a,union(b,c)) we get from dnf (union (inter(a,b),(inter a,c)), its normalize the copmpund exp in logic way
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> =>
    ...
    (texps[texps.length-2]==="is?")? parsePredTExp(texps):
    parseProcTExp(texps);

    const parseDiffTExp = (s:Sexp[]):Result<TExp>=>
        mapv(mapResult(parseTExp,s.slice(1)),(s:TExp[])=> makeDiffTExp(s[0],s[1]));
    
const parsePredTExp = (texps: Sexp[]):Result<TExp>=> {
    const index = texps.indexOf('->');
    return (index === 0) ? makeFailure(`there is no parameters for the predicatretTExp - ${format(texps)}`) :
           (index === texps.length - 1) ? makeFailure(`the predicate return nothing but should return somthing - ${format(texps)}`) :
           bind(parseTupleTExp(texps.slice(0, index)), (args: TExp[]) =>
               mapv( parseTExp(texps[index + 2]), (returnTE: TExp) =>
                    makeProcTExp(args, makeTypePredTExp(returnTE))));
};



const parseInterTExp  = (s:Sexp[]):Result<TExp> =>
    mapv(mapResult(parseTExp,s.slice(1)),(s:TExp[])=>makeInterTExp(s));
...
export const unparseTExp = ( te: TExp): Result<string> => {
    ...
    const up = (x?: TExp):  Result<string | string[]> =>
        ...
        isDiffTExp(x)?  makeOk("(diff "+unparseTExp(x.te1)+" " +unparseTExp(x.te2)+")"):
